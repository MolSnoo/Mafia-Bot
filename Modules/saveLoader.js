...
module.exports.load = async function (game) {
    ...
    for (let i = 0; i < game.players.length; i++) {
        ...
        if (player.alive) {
            player.member.roles.remove(settings.deadRole).catch();
            player.member.roles.add(settings.playerRole).catch();
        }
        else {
    ...
    if (gameJson.poll !== null) {
        ...
        if (gameJson.poll.message !== null)
            game.guild.channels.cache.get(settings.announcementChannel).messages.fetch(gameJson.poll.message).then(message => game.poll.message = message).catch(console.error);
